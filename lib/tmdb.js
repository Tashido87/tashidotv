...
export const IMG = {
  original: (path) => (path ? `/api/tmdb-image/original${path}` : null),
  backdrop: (path) => (path ? `/api/tmdb-image/w1280${path}` : null),
  poster: (path) => (path ? `/api/tmdb-image/w500${path}` : null),
  profile: (path) => (path ? `/api/tmdb-image/w185${path}` : null),
};
...
async function fetchRaw(endpoint, params = {}) {
  // Catalog requests run in client effects. Keep upstream credentials and
  // connections on the server so visitors only need access to this site.
  const query = new URLSearchParams();
  Object.entries(params).forEach(([k, v]) => v != null && query.set(k, v));
  const urlStr = `/api/tmdb${endpoint}?${query}`;
  ...
  try {
    const res = await fetch(urlStr, { signal: AbortSignal.timeout(20000) });

...
async function fetchRaw(endpoint, params = {}) {
  ...
  try {
    const res = await fetch(urlStr, { next: { revalidate: 3600 } });
...
export const getMovieDetails = (id) =>
  tmdbFetch(`/movie/${id}`, { append_to_response: 'credits,videos,similar,images' });

export const getTVDetails = (id) =>
  tmdbFetch(`/tv/${id}`, { append_to_response: 'credits,videos,similar,images' });

...
import { Film, ChevronLeft, ChevronRight, ChevronDown, Filter, Loader2, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
export default function MoviesPage() {
  ...
  useEffect(() => {
    async function loadHeroMovies() {
      try {
        ...
        console.error("Error loading Movie Hero items:", err);
      }
    }
    loadHeroMovies();
  }, []);
  ...
  // Fetch movies whenever page or genre changes
  useEffect(() => {
    async function loadMovies() {
      ...
      try {
        ...
        const [res1, res2] = await Promise.all([
          discoverMedia('movie', { ...params, page: tmdbPageStart }),
          discoverMedia('movie', { ...params, page: tmdbPageEnd })
        ]);
  ...
  const handlePageChange = (newPage) => {
    setPage(newPage);
    const el = document.getElementById('movie-collection');
    if (el) {
  ...
  const selectedGenreName = MOVIE_GENRES.find(g => g.id === selectedGenre)?.name || 'All';
  ...
  return (
    <div className="min-h-screen bg-transparent text-zinc-900 dark:text-white select-none transition-colors duration-300">
      {/* Dynamic Movie Hero Carousel */}
      {heroItems.length > 0 && <Hero items={heroItems} />}

      <div id="movie-collection" className="max-w-[1600px] mx-auto px-6 lg:px-10 pt-16 pb-16">
        {/* Title & Header */}
      <div className="relative z-40 mb-10 animate-fade-in flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-zinc-500 dark:text-white/50 text-[11px] font-bold tracking-widest uppercase mb-1.5">
            <Film className="w-4 h-4 text-zinc-500 dark:text-white/60 animate-pulse" />
          ...
          <p className="mt-3 text-zinc-500 dark:text-white/40 max-w-xl text-[14px] leading-relaxed">
            Explore blockbuster titles, premium action hits, high-scoring dramas, and hand-curated releases sorted by popularity.
          </p>
        </div>
      ...
      <div className="transition-all duration-500">
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
              {items.map((movie, index) => (
                ...
                >
                  <MediaCard item={movie} mediaType="movie" isGrid={true} />
                </div>
              ))}
            </div>
          ...
          /* Empty State */
          <div className="flex flex-col items-center justify-center py-32 text-center space-y-4 bg-zinc-900/10 border border-white/5 rounded-3xl backdrop-blur-md px-6">
            ...
            <div>
              ...
              <p className="text-[12px] text-white/40 mt-1 max-w-xs mx-auto">
